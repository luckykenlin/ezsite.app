...
/**
 * Tenancy for Laravel — see https://tenancyforlaravel.com
 *
 * Trimmed down from the package's published stub: only the events this app
 * actually listens to are declared. The stub's remaining ~50 event keys all
 * mapped to empty listener arrays (a no-op in {@see bootEvents()}), and its
 * commented-out examples belong in the package's own documentation, not here.
 *
 * The stub's ResourceSyncing listeners are gone too: syncing copies central
 * rows INTO per-tenant databases, which this RLS-only app does not have, and
 * no model implements Syncable — the events could never fire. Re-add them
 * from the package stub if that ever changes.
 */
final class TenancyServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        // Livewire's update route must be universal so it resolves on both the
        // central domain (panels) and tenant domains (public site widgets).
