    ...
    <div class="mx-auto flex max-w-2xl flex-col gap-8 px-6">
        ...
        @if ($step === 1)
            <form wire:submit="next" class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    @else
                        @if ($subdomain !== '')
                            <span class="text-sm text-success">{{ __('marketing.wizard.available', ['domain' => $subdomain.'.'.parse_url(config('app.url'), PHP_URL_HOST)]) }}</span>
                        @endif
                    @enderror
                </div>

                <label class="flex flex-col gap-2">
                    <span class="font-semibold">{{ __('marketing.wizard.tagline') }} <span class="font-normal opacity-60">{{ __('marketing.wizard.optional') }}</span></span>
                ...
                <label class="flex flex-col gap-2">
                    <span class="font-semibold">{{ __('marketing.wizard.city') }} <span class="font-normal opacity-60">{{ __('marketing.wizard.optional') }}</span></span>
                ...
                <label class="flex flex-col gap-2">
                    <span class="font-semibold">{{ __('marketing.wizard.phone') }} <span class="font-normal opacity-60">{{ __('marketing.wizard.optional') }}</span></span>
                ...
                <button type="submit" class="btn btn-primary btn-lg self-start">{{ __('marketing.actions.continue') }}</button>
            </form>
        @elseif ($step === 2)
            <form wire:submit="next" class="flex flex-col gap-6">
                ...
                @foreach ($definition->extraFields as $field)
                    ...
                    <label class="flex flex-col gap-2">
                        ...
                        @endif
                        @if ($help) <span class="text-sm opacity-60">{{ $help }}</span> @endif
                    </label>
                @endforeach
                ...
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <button type="button" wire:click="back" class="text-sm opacity-70 hover:opacity-100">{{ __('marketing.actions.back') }}</button>
                </div>
            </form>
        @elseif ($step === 3)
            <form wire:submit="submit" class="flex flex-col gap-6">
                ...
                <div aria-hidden="true" class="hidden">
                    <label>{{ __('marketing.wizard.honeypot') }}<input type="text" wire:model="website" tabindex="-1" autocomplete="off"></label>
                </div>
        ...
        @else
            <div class="flex flex-col items-start gap-6">
                ...
                <h1 class="site-display font-heading">{{ __('marketing.wizard.done.title', ['business' => $businessName]) }}</h1>
                ...
                <p class="site-intro opacity-80">
                    {!! __('marketing.wizard.done.published', ['url' => '<span class="font-semibold">'.e($siteUrl).'</span>']) !!}
                    {{ __('marketing.wizard.done.drafts') }}
                </p>

                <div class="flex flex-wrap items-center gap-4">
                    <a href="{{ $claimUrl }}" class="btn btn-primary btn-lg">{{ __('marketing.wizard.done.open_editor') }}</a>
                    <a href="{{ $siteUrl }}" target="_blank" rel="noopener" class="site-link-cta">{{ __('marketing.wizard.done.view_site') }}</a>
                </div>
            </div>
        @endif
    </div>
