...
@if (filled($phone))
    ...
    <div data-call-bar class="border-base-300 bg-base-100 fixed inset-x-0 bottom-0 z-40 border-t p-3 sm:hidden">
        <div class="flex gap-2">
            <a href="tel:{{ $phone }}" class="site-btn site-btn-primary flex-1"> {{ $capture->callBarLabel() }} </a>
            ...
            @if ($capture->callBarOffersPopup())
                ...
                <button type="button" class="site-btn site-btn-quiet flex-1" data-call-bar-popup hidden>
                    {{ __('Message us') }}
                </button>
            @endif
        </div>
