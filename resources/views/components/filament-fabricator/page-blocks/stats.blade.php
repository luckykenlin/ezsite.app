        ...
        <dl @class(['grid gap-8 text-center', $layout->grid(), 'mt-12' => (bool) $heading])>
            @foreach ($items as $item)
                ...
                <div class="site-stat-item px-4">
                    ...
                    <dd data-editor-field="stats.{{ $loop->index }}.value" class="site-stat text-primary">
                        {{ $item['value'] }}
                    </dd>
                    <dt data-editor-field="stats.{{ $loop->index }}.label" class="mt-2 font-semibold">
                        {{ $item['label'] }}
                    </dt>
                    @if ($item['description'] ?? null)
                        <dd data-editor-field="stats.{{ $loop->index }}.description" class="mt-1 text-sm opacity-60">
                            {{ $item['description'] }}
                        </dd>
                    @endif
                </div>
