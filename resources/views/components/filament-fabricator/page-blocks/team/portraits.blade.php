        ...
        <div class="mt-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($items as $item)
                ...
                <div class="text-center">
                    ...
                    @else
                        <span
                            class="mx-auto flex size-28 items-center justify-center rounded-full bg-base-300 text-3xl font-bold text-base-content/50"
                            aria-hidden="true"
                        >{{ $initials }}</span>
                    @endif
                    ...
                    @endif
                    @if ($item['bio'] ?? null)
                        <p class="mt-2 text-base-content/70">{{ $item['bio'] }}</p>
                    @endif
                </div>
            @endforeach
        </div>
