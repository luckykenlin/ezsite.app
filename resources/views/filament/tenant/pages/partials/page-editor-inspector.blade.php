...
<div class="pe-pane pe-inspector">
    {{-- Two faces, one column. Site Styles is a site-wide concern, which this
         pane already owns at its foot ("Site-wide → Header / Footer"), and it
         is judged against the canvas — so it belongs beside the canvas, not in
         a modal on top of it and not in a fourth grid track the canvas has no
         room for. --}}
    <div class="pe-tabs" role="tablist">
        <button
            ...
            class="pe-tab"
            role="tab"
            aria-selected="{{ $this->showingSiteStyles ? 'false' : 'true' }}"
            @unless ($this->showingSiteStyles) data-active @endunless
            wire:click="showPageInspector"
        >
            {{ __('Page') }}
        </button>

        {{-- Design tokens live on the Business row, so the tab is only offered
             once there is one — same gate the Design modal carried before it. --}}
        @if ($this->hasBusinessProfile())
            <button
                type="button"
                class="pe-tab"
                role="tab"
                aria-selected="{{ $this->showingSiteStyles ? 'true' : 'false' }}"
                @if ($this->showingSiteStyles) data-active @endif
                wire:click="showSiteStyles"
            >
                {{ __('Site styles') }}
                @if ($this->hasStagedStyles())
                    <span class="pe-tab-dot" title="{{ __('Unapplied style changes') }}"></span>
                @endif
            </button>
        @endif
    </div>
    ...
    @else
        ...
        @else
            ...
            <div wire:key="block-form-{{ $this->selectedBlockKey }}">{{ $this->blockForm }}</div>
        @endif
        ...
        <div>
            ...
            <div class="pe-chrome-links">
                @foreach ([ChromeSlot::Header, ChromeSlot::Footer] as $slot)
                    <button type="button" class="pe-chrome-link" wire:click="selectBlock('{{ $slot->editorKey() }}')">
                        <x-filament::icon
