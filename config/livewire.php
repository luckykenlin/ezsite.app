...
return [
    ...
    */
    ...
    /*
     * Tenancy-critical (media uploads): the upload endpoint is its own HTTP
     * request, so without the tenancy middleware it runs in CENTRAL context
     * — the temp file lands under the central storage root while the
     * component's (tenant-context) request looks for it under
     * storage/{suffix}{tenant}/…, and the upload silently fails. 'universal'
     * lets the same endpoint serve both contexts.
     *
     * The disk is pinned to 'public' for the same reason FILESYSTEM_DISK
     * can't be trusted here: only the disks listed in
     * tenancy.filesystem.disks are tenant-suffixed.
     */
