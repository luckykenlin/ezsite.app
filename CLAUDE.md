...
Single database + Postgres RLS for tenant isolation — **not** one database
per tenant. `DatabaseTenancyBootstrapper` is intentionally disabled;
`PostgresRLSBootstrapper` is used instead.

- RLS policies are auto-generated by `tenants:rls` and re-synced automatically
  after every pgsql migration (`TenancyServiceProvider::syncRlsPoliciesAfterMigrations`).
  Never hand-write RLS policies.
- To exclude a `tenant_id` column from RLS (e.g. must be queryable before
  tenancy resolves, like `domains.tenant_id`), add `->comment('no-rls')` to
  that column in its migration.
- Routes: `routes/web.php` = central-domain only. `routes/tenant.php` = tenant
  routes, mounted separately via the `tenant` middleware group by
